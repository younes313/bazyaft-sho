...
from .serializers import KhanevarEmailRegisterSerializer , EdariTypeAddSerializer
from .models import EdariType
...
@permission_classes((IsAuthenticated,))
class EdariTypeAdd(APIView):
    serializer_class = EdariTypeAddSerializer

    def post(self, request, format=None):
        serializer = EdariTypeAddSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            dic = serializer.data
            dic.update({'status':'101'})
            ...
            dic = {'status':[],}
            for err in serializer.errors['name']:
                if str(err) ==  "edari type with this name already exists.":
                    dic['status'].append('102')
                if str(err) ==  "Ensure this field has no more than 50 characters.":
                    dic['status'].append('104')
                if str(err) ==  "This field may not be blank.":
                    dic['status'].append('105')
